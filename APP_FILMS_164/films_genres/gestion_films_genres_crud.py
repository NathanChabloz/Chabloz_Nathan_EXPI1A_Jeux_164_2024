        ...
        try:
            with DBconnection() as mc_afficher:
                ...
                if id_film_sel == 0:
                    ...
                    strsql_genres_films_afficher_data += """ HAVING id_jeux= %(value_id_film_selected)s"""
...
def edit_genre_film_selected():
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_afficher = """SELECT id_categorie, nom_categorie FROM t_catégorie ORDER BY id_categorie ASC"""
            ...
            lst_data_film_selected = [item['id_jeux'] for item in data_genre_film_selected]
...
def update_genre_film_selected():
    if request.method == "POST":
        try:
            ...
            strsql_insert_genre_film = """INSERT INTO t_catégorie_jeux (id_catégorie_jeux, fk_catégorie, fk_jeux)
            ...
            strsql_delete_genre_film = """DELETE FROM t_catégorie_jeux WHERE fk_catégorie = %(value_fk_genre)s AND fk_jeux = %(value_fk_film)s"""
...
def genres_films_afficher_data(valeur_id_film_selected_dict):
    ...
    try:
        ...
        strsql_film_selected = """SELECT id_jeux, nom_jeu, durée_moyenne, joueurs_min, joueurs_max, image_jeux, date_sortie_jeux, age_min, age_max,GROUP_CONCAT(id_categorie) as GenresFilms FROM t_catégorie_jeux
        ...
        strsql_genres_films_non_attribues = """SELECT id_categorie, nom_categorie FROM t_catégorie WHERE id_categorie not in(SELECT id_categorie as idGenresFilms FROM t_catégorie_jeux
                                                    INNER JOIN t_jeux ON t_jeux.id_jeux = t_catégorie_jeux.fk_jeux
                                                    INNER JOIN t_catégorie ON t_catégorie.id_categorie = t_catégorie_jeux.fk_catégorie
                                                    WHERE id_jeux = %(value_id_film_selected)s)"""

        strsql_genres_films_attribues = """SELECT id_jeux, id_categorie, nom_categorie FROM t_catégorie_jeux
                                            INNER JOIN t_jeux ON t_jeux.id_jeux = t_catégorie_jeux.fk_jeux
                                            INNER JOIN t_catégorie ON t_catégorie.id_categorie = t_catégorie_jeux.fk_catégorie
                                            WHERE id_jeux = %(value_id_film_selected)s"""
